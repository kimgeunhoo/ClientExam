...
public class QuestDialogueUI : MonoBehaviour
{
    ...
    private void Refresh()
    {
        ...
        requestPanel.SetActive(!canReward);
        rewardPanel.SetActive(canReward);

        acceptButton.gameObject.SetActive(!hasQuest);
        getRewardButton.gameObject.SetActive(canReward);
        ...
        dialogueText.text = "이미 보상은 줬네. 고생했네.";
        requestPanel.SetActive(false);
        rewardPanel.SetActive(false);
        acceptButton.gameObject.SetActive(false);
        getRewardButton.gameObject.SetActive(false);
